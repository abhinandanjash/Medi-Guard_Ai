...
NEW_INIT_LANDING_PAGE = """function initLandingPage() {
    ...
    document.fonts.ready.then(function() {
        ...
        function frame() {
            ...
            bgGrad.addColorStop(0.3, '#040b17'); 
            bgGrad.addColorStop(0.8, '#010305'); 
            ...
            ctx.save();
            // EXTREMELY CRITICAL CHANGE: Removing additive 'lighter' mode to fix haziness.
            // Cubes maintain exact crisp solid edges and block each other properly.
                    ...
                    if (p.isText) {
                        ...
                        if (activeTime > 0) {
                            ...
                            if (p.lpx !== 0 && dt < T_ASSEMBLE && p.z > -FL + 10) {
                                ...
                                if (speed > 4) {
                                    // Make streaks temporarily additive for glow, then immediately restore 'source-over'
                        ...
                        if (p.lpx !== 0 && scale > 0) {
                            ...
                            ctx.translate(p.px, p.py);
                            ctx.rotate(p.rot);
                            // Background particles slightly additive for a ghostly look
                            ctx.globalCompositeOperation = 'lighter'; 
