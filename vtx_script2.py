    ...
    document.fonts.ready.then(function() {
        ...
        for (var i = 0; i < N; i++) {
            ...
            particles.push({
                ...
                delay: Math.random() * 3000, // 3 full seconds of staggering for much slower staggered start
                sz: 3 + Math.random() * 4,
                rotSpeed: (Math.random() - 0.5) * 0.02, // Base rotation speed
        ...
        function playCinematicAudio() {
            try {
                ...
                // THX Chord Riser extended to 8.0 seconds
                ...
                noiseFilter.frequency.exponentialRampToValueAtTime(4000, ac.currentTime + 7.8);
                
                var noiseGain = ac.createGain(); 
                noiseGain.gain.setValueAtTime(0, ac.currentTime);
                noiseGain.gain.linearRampToValueAtTime(0.6, ac.currentTime + 5.0); 
                noiseGain.gain.exponentialRampToValueAtTime(1.0, ac.currentTime + 7.8);
        ...
        function frame() {
            ...
            ctx.save();
            // Lighter blending mode gives glowing overlapping cubes
                ...
                for (var pi = 0; pi < particles.length; pi++) {
                    ...
                    if (p.isText) {
                        ...
                        if (dt < T_ASSEMBLE) {
                            ...
                            // Exponentially slowed pacing so users can admire the physics
                            var t = 1 - Math.pow(1 - fp, 3);
                            var u = 1 - t;

                            // ── COMPLEX NEW MOVEMENTS ──
                            // 1. Serpentine Wobble math (makes them slither through space like a swarm rather than smooth arching)
                            ...
                            // 2. Tumble & Spin math! Cubes spin wildly while flying, gracefully locking to 0 radians as they seat.
                        ...
                        if (activeTime < 700) renderAlpha *= (activeTime / 700); // Slower fade in
                        ...
                        if (activeTime > 0) {
                            ...
                            // Inject 2D Canvas Rotation for spinning tumbling cubes
                            ctx.save();
            ...
            // Update Progress HUD to account for 8 second boot
            if (dt < T_ASSEMBLE) {
                ctx.save();
