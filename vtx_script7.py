...
NEW_INIT_LANDING_PAGE = """function initLandingPage() {
    ...
    // --- NEW: LOAD EARTH BACKGROUND IMAGE ---
    var bgImg = new Image();
    bgImg.src = '/static/earth.jpg'; 
    ...
    function generateUltraPremiumCubeCanvas(hue) {
        ...
        gT.addColorStop(0, `hsla(${hue}, 40%, 100%, 0.95)`); 
    ...
    document.fonts.ready.then(function() {
        ...
        function frame() {
            ...
            if (bgImg.complete && bgImg.naturalWidth !== 0) {
                ...
                ctx.save();
                ctx.translate(W/2 + shakeX*0.1, H/2 + shakeY*0.1); 
                var bgCamScale = 1 + (dt / T_EXIT) * 0.05; 
                ctx.scale(bgCamScale, bgCamScale);
                ...
                bgGrad.addColorStop(0.4, '#020510'); 
                bgGrad.addColorStop(0.8, '#010103'); 
            ...
            // --- DRAW 3D BACKGROUND GRID OVERLAY ---
            ctx.save();
            var gScale = 1 + Math.sin(dt*0.0005)*0.1;
            ctx.scale(gScale, gScale);
            var gridAlpha = (bgImg.complete && bgImg.naturalWidth !== 0) ? 0.015 : 0.03;
            if (dt < T_ASSEMBLE) gridAlpha += Math.sin(dt*0.002)*0.015;
            ctx.strokeStyle = `rgba(0, 240, 255, ${gridAlpha})`;
            ctx.lineWidth = 1;
            var gap = 150;
            var ext = Math.max(W, H) * 1.5;
            ctx.beginPath();
            for(var x = -ext; x <= ext; x+=gap) { ctx.moveTo(x, -ext); ctx.lineTo(x, ext); }
            for(var y = -ext; y <= ext; y+=gap) { ctx.moveTo(-ext, y); ctx.lineTo(ext, y); }
            ctx.stroke();
            ctx.restore();

            // --- DRAW CINEMATIC SPACE STARFIELD ---
            ctx.globalCompositeOperation = 'lighter';
            for (var k = 0; k < stars.length; k++) {
                var s = stars[k];
                s.z += s.vz;
                if (s.z < -FL + 10) { s.z = 5000; s.x = (Math.random() - 0.5) * W * 5; s.y = (Math.random() - 0.5) * H * 5; }
                
                var starScale = FL / (FL + Math.max(1, s.z));
                var spx = s.x * starScale;
                var spy = s.y * starScale;
                
                var twinkle = 0.5 + Math.sin(dt * s.twinkleSpeed) * 0.5 * s.twinkleAmp;
                var sAlpha = Math.min(1.0, starScale * 2.0) * twinkle;
                if (sAlpha < 0.05) continue;

                var renderSize = Math.max(0.5, 3.0 * starScale);
                ctx.fillStyle = `hsla(${s.hue}, 80%, 100%, ${sAlpha})`;
                ctx.beginPath(); ctx.arc(spx, spy, renderSize, 0, Math.PI*2); ctx.fill();
            }
            ...
            if (particleAlphaMaster > 0) {
                for (var pi = 0; pi < particles.length; pi++) {
                    ...
                    if (p.isText) {
                        ...
                        if (activeTime > 0) {
                            var w = Math.max(0.1, p.sz * depthScale) * 0.8; 
                            ...
                            if (p.lpx !== 0 && dt < T_ASSEMBLE && p.z > -FL + 10) {
                                ...
                                if (speed > 4) {
                                    ...
                                    ctx.lineWidth = w * 0.5; ctx.strokeStyle = `hsla(${p.hue}, 100%, 65%, ${0.6 * renderAlpha})`;
                                    ctx.stroke();
                                    ctx.globalCompositeOperation = 'source-over';
                                }
                            }
