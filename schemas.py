...
class UserRegister(BaseModel):
    ...
    organization_name: Optional[str] = None  # For hospital/pharma
...
# ── Hospital & Pharma Submission Schemas ──────────────────────

class HospitalSubmissionRequest(BaseModel):
    patient_code: str
    submission_type: str  # 'bill' | 'report' | 'blood_report' | 'prescription'
    title: str
    description: Optional[str] = None
    amount: Optional[float] = None      # For bills
    file_data: Optional[str] = None     # Base64 for report documents
    patient_admitted: bool = False

class HospitalSubmissionResponse(BaseModel):
    success: bool
...
class PatientHistoryRequest(BaseModel):
    ...
    genetic_diseases: Optional[str] = None
...
class SendSummaryRequest(BaseModel):
    patient_code: str
    doctor_name: str   # Match by doctor full_name
    summary_text: str
